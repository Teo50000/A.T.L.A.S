...
def route(intent, text):
    ...
    elif intent == "leerArchivo":
        if archivos:
            for arch in archivos:
                print(actions.leer(arch))
        else:
            print("[Router] No se encontró archivo para leer.")
        return

    elif intent == "buscarArchivo":
        if archivos:
            for arch in archivos:
                ruta = actions.encontrArchPorNombre(arch)
                print(f"Archivo encontrado en: {ruta}" if ruta else "Archivo no encontrado.")
        else:
            print("[Router] No se indicó nombre de archivo.")
        return

    elif intent == "buscarCarpeta":
        if carpetas:
            for carp in carpetas:
                ruta = actions.encontrarCarPorNombre(carp)
                print(f"Carpeta encontrada en: {ruta}" if ruta else "Carpeta no encontrada.")
        else:
            print("[Router] No se indicó carpeta.")
        return

    elif intent == "listarTipo":
        if textos and carpetas:
            tipo = textos[0]
            ruta = carpetas[0]
            print(actions.encontrartipoencarpeta(tipo, ruta))
        else:
            print("[Router] Faltan datos: tipo de archivo o carpeta.")
        return

    elif intent == "leerPDF":
        if archivos:
            for arch in archivos:
                print(actions.leerPDF(arch))
        else:
            print("[Router] No se encontró PDF para leer.")
        return
